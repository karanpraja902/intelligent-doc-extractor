...
const SchemaBuilder: React.FC<SchemaBuilderProps> = ({ fields, setFields }) => {
  ...
  const handleUpdateArrayField = (fieldId: string, oldKey: string, newKey: string, description: string) => {
    setFields(fields.map(f => {
      if (f.id === fieldId && f.items_structure) {
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {fields.length === 0 ? (
          <div className="text-center py-10 px-4 border-2 border-dashed border-slate-200 rounded-lg">
            ...
            <button onClick={handleAddField} className="text-blue-500 hover:underline text-sm font-medium">
            ...
            >
              <div className="flex gap-3 items-start">
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-500 mb-1">Description (for AI context)</label>
                    <input
                      type="text"
                    ...
                    <div className="mt-3 border border-blue-100 bg-blue-50/30 rounded-lg p-3">
                      ...
                      >
                        <span className="flex items-center gap-1.5">
                          ...
                          Array Item Structure ({Object.keys(field.items_structure || {}).length} fields)
                        </span>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      ...
                      <span className={`text-xs font-medium ${field.required ? 'text-slate-700' : 'text-slate-400'}`}>
                        Required
                      </span>
                    </label>
